...
let claudeChatFocusService = null;
...
module.exports = {
  ...
  /**
   * Invalidates the icon cache for a specific file path.
  ...
  /**
   * Consumes the claude-chat-focus service for setting focus context.
   * @param {Object} service - The claude-chat-focus service object
   * @returns {Object} Disposable to unregister the service
   */
  consumeClaudeChatFocus(service) {
    claudeChatFocusService = service;
    if (this.pathList) {
      this.pathList.setClaudeChatFocusService(service);
    }
    return {
      dispose: () => {
        claudeChatFocusService = null;
      },
    };
  },

  /**
   * Gets the claude-chat-focus service.
   * @returns {Object|null} The claude-chat-focus service or null
   */
  getClaudeChatFocusService() {
    return claudeChatFocusService;
  },
};
