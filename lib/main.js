...
module.exports = {
  ...
  globPromise(pPath) {
    return new Promise((resolve) => {
      const task = Task.once(require.resolve("./scan"), pPath, this.ignores);
      task.on('fuzzy-files:entries', (entries) => {
        for (let fPath of entries) {
          if (fPath === ".") continue;
  ...
  performAction(mode, params) {
    ...
    if (mode === "open") {
      atom.workspace.open(item.aPath, { initialLine: this.initialLine, pending: atom.config.get("core.allowPendingPaneItems") });
