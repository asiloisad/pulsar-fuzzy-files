...
module.exports = class PathList {
    ...
    this.disposables.add(
      ...
      atom.commands.add(this.slv.element, {
        ...
        "fuzzy-files:cut-file": () =>
          this.didConfirmSelection("clip", { effect: "cut" }),
        "fuzzy-files:copy-file": () =>
          this.didConfirmSelection("clip", { effect: "copy" }),
      }),
  ...
  elementForItem(item, options) {
    ...
    priBlock.classList.add(
      "icon",
      "icon-line",
      ...this.S.iconClassForPath(item.aPath)
    );
    ...
    if (this.projectCount > 1) {
      ...
      secBlock.textContent = item.pPath;
  ...
  filter(items, query) {
    ...
    for (let item of items) {
      item.score =
        atom.ui.fuzzyMatcher.score(item.text, this.query) / item.distance;
