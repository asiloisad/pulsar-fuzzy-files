...
export default class PathList {
  ...
  elementForItem (item, options) {
    ...
    if (this.S.addIconToElement) {
      priBlock.classList.add('icon', 'icon-line')
      this.S.addIconToElement(priBlock, path.join(item.pPath, item.fPath))
    } else {
      priBlock.classList.add('icon', 'icon-line', 'icon-file-text')
    }
